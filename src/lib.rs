...
impl SystemGeom {
    ...
    fn add_com(
        ...
        let mut com: Vec<rao::Actuator> = com_coords
            .iter()
            .map(move |p| {
                let x: f64 = (p.x * dm_clocking.cos() + p.y * dm_clocking.sin()) * (1.0 + dm_zoom)
                    + dm_delta.0;
                let y: f64 = (-p.x * dm_clocking.sin() + p.y * dm_clocking.cos()) * (1.0 + dm_zoom)
                    + dm_delta.1;
